...
class SteeringLayout(Widget):
  ...
  def _initialize_items(self):
    ...
    self._htd_resume_angle_diff_threshold = option_item_sp(
      param="dp_htd_resume_angle_diff_threshold",
      title=lambda: tr("Resume angle difference"),
      description=lambda: tr("Resume when model expected angle and steering angle difference is below this value."),
      min_value=5,
      max_value=20,
      value_change_step=1,
      label_callback=lambda angle: f'{angle} °',
    ...
    self._htd_resume_delay = option_item_sp(
      ...
      description=lambda: tr("Delay before lateral control resumes after HTD release condition is met."),
      min_value=0,
      max_value=5000,
    ...
    items = [
      ...
      self._htd_resume_angle_diff_threshold,
  ...
  def _update_state(self):
    ...
    # 保持 HTD 子項可見，僅在總開關關閉時置灰。
    self._htd_enabled.action_item.set_enabled(ui_state.is_offroad())
    self._htd_turn_angle_threshold.action_item.set_enabled(ui_state.is_offroad() and htd_is_enabled)
    self._htd_resume_angle_diff_threshold.action_item.set_enabled(ui_state.is_offroad() and htd_is_enabled)
    self._htd_resume_delay.action_item.set_enabled(ui_state.is_offroad() and htd_is_enabled)
    # ----------------------
