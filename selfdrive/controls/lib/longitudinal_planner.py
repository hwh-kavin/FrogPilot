...
class LongitudinalPlanner(LongitudinalPlannerSP):
  ...
  def update(self, sm):
    ...
    # 傳入覆寫參數給 MPC (使用計算過後的 personality)
    self.mpc.update(sm['radarState'], v_cruise, personality=personality, a_cruise_min_override=a_cruise_min_override)

    self.v_desired_trajectory = np.interp(CONTROL_N_T_IDX, T_IDXS_MPC, self.mpc.v_solution)
    self.a_desired_trajectory = np.interp(CONTROL_N_T_IDX, T_IDXS_MPC, self.mpc.a_solution)

    # ACM 模組介入：滑行與柔和煞車 (使用計算過後的 personality)
    mode = 'blended' if self.is_e2e(sm) else 'acc'
    self.acm.update_states(sm['carControl'], sm['radarState'], reset_state, v_ego, v_cruise, mode=mode, personality=personality)
